...
import '../app_lifecycle/er_app_lifecycle.dart';
import '../settings/settings.dart';
import 'songs.dart';
import 'sounds.dart';

/// Allows playing music and sound. A facade to `package:audioplayers`.
class ERAudioController {
  ...
  ERSettingsController? _settings;
  ...
  ERAudioController({int polyphony = 2})
      : assert(polyphony >= 1),
  ...
  /// [ERSettingsController.audioOn] is `true` or if its
  /// [ERSettingsController.soundsOn] is `false`.
  void playSfx(SfxType type) {
    ...
    currentPlayer.play(AssetSource('sfx/endless_runner/$filename'),
        volume: soundTypeToVolume(type));
    _currentSfxPlayer = (_currentSfxPlayer + 1) % _sfxPlayers.length;
  }

  /// Enables the [ERAudioController] to listen to [AppLifecycleState] events,
  /// and therefore do things like stopping playback when the game
  /// goes into the background.
  void _attachLifecycleNotifier(ERAppLifecycleStateNotifier lifecycleNotifier) {
    _lifecycleNotifier?.removeListener(_handleAppLifecycle);
  ...
  /// Enables the [ERAudioController] to track changes to settings.
  /// Namely, when any of [ERSettingsController.audioOn],
  /// [ERSettingsController.musicOn] or [ERSettingsController.soundsOn] changes,
  /// the audio controller will act accordingly.
  void _attachSettings(ERSettingsController settingsController) {
  ...
  Future<void> _playCurrentSongInPlaylist() async {
    ...
    try {
      await _musicPlayer.play(AssetSource('music/endless_runner/${_playlist.first.filename}'));
    } catch (e) {
  ...
  Future<void> _preloadSfx() async {
    ...
    await AudioCache.instance.loadAll(SfxType.values
        ...
        .map((path) => 'sfx/endless_runner/$path')
        .toList());
  }
